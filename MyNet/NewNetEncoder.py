...
trainImagePath = './SkyBoxAddMask'
targetImagePath = "./skybox"

trainDataset = tf.data.Dataset.list_files(targetImagePath + '/*.exr', shuffle=False)
# trainTargetDataset = tf.data.Dataset.list_files(targetImagePath + '/*.exr', shuffle=False)
...
# for i in trainTargetDataset:
#     t = load(i.numpy())
#     targetList.append(t)

trainDataset = tf.data.Dataset.from_tensor_slices(trainList)
# trainTargetDataset = tf.data.Dataset.from_tensor_slices(targetList)
...
#     #print(item.shape)
#     trainDataMean+=tf.reduce_sum(item)
# trainDataMean/=32*128*3*num
# for item in trainDataset.take(num):
#     x=item-trainDataMean
#     x=x**2
#     trainDataStd+=tf.reduce_sum(x)
# trainDataStd=math.sqrt(trainDataStd/(32*128*3*num))
# print("trainDataMean: ",trainDataMean)
# print("trainDataStd: ",trainDataStd)
...
# validationImagePath=PATH+"validation3500"
#
# validationDataset=tf.data.Dataset.list_files(validationImagePath+'/*.exr')
# validationList=[]
# for i in validationDataset:
#     tmp=load(i.numpy())
#     validationList.append(tmp);
# maxV=-1e9
# minV=1e9
# for i in range(len(validationList)):
# #    v=normalize(testList[i])
#     v=tf.constant(validationList[i])
#     maxV=max(tf.reduce_max(v),maxV)
#     minV=min(tf.reduce_min(v),minV)
#
# print("Max:",maxV)
# print("Min:",minV)
# validationDataset=tf.data.Dataset.from_tensor_slices(validationList)
...
checkpoint_dir = './NoMask-checkPoint'
...
checkpoint.restore(tf.train.latest_checkpoint('./NoMask-checkPoint'))
...
def generate_images(test_input, distorted_test_input):
    ...
    display_list = [test_input[0, ...], distorted_test_input[0, ...], deNormalize(decoder_output[0, ...]),
                    deNormalize(decoder_output_distorted[0, ...])]
...
@tf.function
def train_step(input_image, distorted_input_image):
    ...
    with tf.GradientTape() as encoder_tape, tf.GradientTape() as decoder_tape:
        ...
        ls = loss_s(input_image, decoder_output, decoder_output_distorted, encoder_output, encoder_output_distorted)
        ld = loss_d(encoder_output, encoder_output_distorted)
        lr = loss_r(input_image, decoder_output, decoder_output_distorted)
...
@tf.function
def validation_step(input_image, distorted_input_image):
    input_image = normalize(input_image)
    distorted_input_image = normalize(distorted_input_image)
...
@tf.function
def test_step(input_image, distorted_input_image):
    input_image = normalize(input_image)
    distorted_input_image = normalize(distorted_input_image)
    encoder_output = encoder(input_image, training=False)
    encoder_output_distorted = encoder(distorted_input_image, training=False)
...
def train(dataset, epochs):
    ...
    for epoch in range(1, epochs):
        ...
        for input_image in dataset:
            train_step(input_image, radiometric_distortions(input_image))
        ...
        # modify temporary omit
        # for input_image in validationDataset:
        #   validation_step(input_image, radiometric_distortions(input_image))
        ...
        #
        # for input_image in testDataset:
        #   test_step(input_image, radiometric_distortions(input_image))
        ...
        for inp in trainDataset.take(1):
            generate_images(inp, radiometric_distortions(inp))
...
tf.saved_model.save(encoder, "./Model_NoMask/Encoder")
tf.saved_model.save(decoder, "./Model_NoMask/Decoder")
